...
define(['faceitConsApp' + window.__env.minUrl
    ...
    app.controller("interviewerController",
        ['$scope', '$rootScope', '$routeParams', '$location', 'sharedService', 'sharedMethod', 'storageFactory',
            function ($scope, $root, $routeParams, $location, ajx, $support, $shared) {
                $scope.Results = [];
                $scope.ResultsBackup = [];
                ...
                $scope.loadResults = function () {
                    ...
                    ajx.GetAllInterviewers(function (res) {
                        $support.stop();
                        if (res.status === 200) {
                ...
                $scope.ShowHideCandidate = function (bshow) {
                    ...
                    if (bshow) {
                        $support.Show('popAddNewInterviewer');
                    } else {
                        $support.Hide('popAddNewInterviewer');
                    }
                };
                ...
                $scope.SubmitCandidate = function () {
                    ...
                    if ($support.IsNull($scope.CanName) ||
                        ...
                        $support.SetText('errEvaAdd', 'All fields are mandatory');
                        $support.Show('errEvaAdd');
                        return;
                    }

                    if (!$support.IsValidEmail($scope.CanEmail)) {
                        $support.SetText('errEvaAdd', 'Enter valid e-mail');
                        $support.Show('errEvaAdd');
                        return;
                    }
                    ...
                    ajx.AddInterviewer(data, function (res) {
                        $support.stop();
                        if (res.status === 200) {
                            if (parseInt(res.data.Results) === 0) {
                                $support.SetText('errEvaAdd', 'E-mail already registered.');
                                $support.Show('errEvaAdd');
                            }
                            else if (parseInt(res.data.Results) === -1) {
                                $support.SetText('errEvaAdd', 'System fail to process. Please try again.');
                                $support.Show('errEvaAdd');
                            } else {
                                $scope.loadResults();
                                $scope.ShowHideCandidate(false);
                                $support.Show('popAddInterviewerSuccessful');
                            }
                        }
                    }, function (err) {
                        $support.stop();
                ...
                $scope.ShowNewScheduledSession = function (keyId) {
                    $root.$broadcast('NewSession', { JobId: 0, CanId:0, EvaId: keyId });
                };
            }]);
});