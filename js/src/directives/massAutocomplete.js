            ...
            controller: ['$scope', function ($scope) {
                ...
                function _suggest(term, target_element) {
                    ...
                    if (typeof (term) === 'string' && term.length > 0) {
                        $q.when(current_options.suggest(term, current_element_delimiter),
                ...
                function _attach(ngmodel, target_element, options,
                    items, keyText, itemLink, isdelimiter) {
                    ...
                    if (options.on_attach) {
                        options.on_attach(props);
                    }

                    current_element = target_element;
                    current_element_delimiter = isdelimiter;
                ...
                function update_model_value(value) {
                    if (current_model.$modelValue !== value) {
                        ...
                        var elmModelName = current_element[0].attributes['ng-model'].value;
                        $scope.$parent[elmModelName] = value;
                    }

                    if (current_options.on_update) {
                        current_options.on_update(value);
                    }
                }
